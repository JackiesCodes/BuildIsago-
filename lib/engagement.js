...
/**
 * Which of the managed sections this viewer may not open.
 *
 * No longer used to hide tabs — the client portal stopped having them —
 * but it is still the access gate on /invoices, /approvals and
 * /retainers, which are reachable by direct link so a bill the studio
 * sends still opens. The safety net is the point: a section is closed off
 * only when it is also empty, so a self-serve client who genuinely has an
 * invoice can still reach it.
 */
