...
/** Same idea, for the public venture-logos bucket. */
export function publicVentureLogoUrl(supabase, path) {
  if (!path) return null;
  return supabase.storage.from('venture-logos').getPublicUrl(path).data.publicUrl;
}
