...
export async function refundEnrollment(enrollmentId) {
  const { user, profile, supabase } = await getSessionProfile();
  if (!user || profile?.role !== 'studio') redirect('/login');
  ...
  const { data: enrollment, error: fetchError } = await supabase
    .from('course_enrollments')
    .select('status, stripe_payment_intent_id, amount_paid')
    .eq('id', enrollmentId)
    .single();
  if (fetchError || !enrollment) return { error: 'Enrollment not found.' };
  if (enrollment.status !== 'paid') return { error: 'This enrollment is not in a refundable state.' };

  if (!enrollment.stripe_payment_intent_id) {
    if (Number(enrollment.amount_paid) === 0) {
      const { error } = await supabase
        .from('course_enrollments')
        .update({ status: 'refunded', refunded_at: new Date().toISOString() })
        .eq('id', enrollmentId);
      if (error) return { error: error.message };
      await logAudit(supabase, 'course_enrollment.revoked', 'course_enrollments', enrollmentId, { free: true });
      paths();
      return { error: null };
    }
    return { error: 'No Stripe payment is associated with this enrollment.' };
  }
  ...
  let refund;
  try {
    refund = await stripe.refunds.create({ payment_intent: enrollment.stripe_payment_intent_id });
  } catch (err) {
    await logError('refundEnrollment', err, { enrollmentId });
  ...
  await logAudit(supabase, 'course_enrollment.refunded', 'course_enrollments', enrollmentId, { stripe_refund_id: refund.id });
